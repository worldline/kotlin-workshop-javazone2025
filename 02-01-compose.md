...
Thus, it really allows Kotlin to be a *write once, run everywhere* language.
...
1. Let's finish by styling the list items by adding a padding to the `Text` inside the `LazyColumn`.
   1. Modify the `Text` inside the `LazyColumn` to:
       ```kotlin
...
In addition to that, since Compose Multiplatform is based on Jetpack Compose, you can even refer to [Jetpack Compose documentation](https://developer.android.com/develop/ui/compose/documentation) for additional documentation.